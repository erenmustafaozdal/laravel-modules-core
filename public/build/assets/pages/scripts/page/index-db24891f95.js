...
var Index = {
    ...
    init: function (relation)
    {
        ...
        if (typeof(relation) == 'undefined') {
            ModelIndex.exportColumns = [2,3,4,5,6,7];
            ModelIndex.relation = null;
            var columns = ModelIndex.columns;
            ModelIndex.columns = [];
            $.each(columns, function(index)
            {
                if (index === 5) {
                    ModelIndex.columns.push({
    ...
    handleDatatable: function()
    {
        ...
        var exportColumns = this.exportColumns;
            ...
            actionButtonCallback: function(Editor)
            {
                ...
                $script.ready('form', function()
                {
                    ModelForm.init({
                        ...
                        {
                            var category = relation === null ? validation.form.find('select[name="category_id"]').val() : validation.form.find('input[name="category_id"]').val();
                            var datas = {
